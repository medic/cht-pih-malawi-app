...
const { muteScenarios } = require('../form-inputs');
const formName = 'mute';
const harness = new TestHarness();

describe('Mute form test', () => {
    before(async () => {
    ...
    it('mute form can be loaded', async () => {
        await harness.loadForm(`${formName}`);
        expect(harness.state.pageContent).to.include(`${formName}`);
    });

    it('mute form can be filled and successfully saved - reason: {in list} ', async () => {
        // Load the mute form and fill in
        const result = await harness.fillForm(...muteScenarios.notOther);
    ...
    it('mute form can be filled and successfully saved - reason: other ', async () => {
        // Load the mute form and fill in
        await harness.loadForm(`${formName}`);
